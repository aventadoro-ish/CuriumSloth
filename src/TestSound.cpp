...
void runTestSound() {  // New wrapper function
    ...
    if ((save == 'y') || (save == 'Y')) {
        ...
        if (!f) {
            ...
            free(iBigBufNew);  // Free memory before return
            return;
        }
    ...
    free(iBigBufNew);  // Free allocated memory for iBigBufNew
    printf("\n");
    system("pause");
}
